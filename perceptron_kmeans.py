...
def run_perceptron_kmeans_search(min_clusters=3, max_clusters=25):
    ...
    # Scale the base features (Mandatory for K-Means and Perceptron)
    ...
    plt.plot(k_values, accuracies, marker='v', linewidth=2, markersize=8, color='#f39c12', label='Perceptron + K-Means Centroid Distances')
    
    # Plot the baseline
    plt.axhline(y=baseline_acc, color='#2c3e50', linestyle='--', linewidth=2, label=f'Baseline Perceptron ({baseline_acc:.2f}%)')
    
    # Aesthetics
    plt.title('Perceptron Accuracy vs. K-Means Geometric Augmentation', fontsize=16, pad=15)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'perceptron_kmeans_search.png', dpi=300)
