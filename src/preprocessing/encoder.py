...
def encode_categorical_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1. High-cardinality strings to drop (too many unique values for One-Hot Encoding)
    # tourney_date is handled in feature engineering; tourney_name causes bloat.
    cols_to_drop = ['tourney_name', 'tourney_date'] 
    data = data.drop(columns=[c for c in cols_to_drop if c in data.columns])

    # 2. One-Hot Encode low-cardinality categorical variables
