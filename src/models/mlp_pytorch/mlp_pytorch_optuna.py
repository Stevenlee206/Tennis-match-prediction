...
def objective(trial, X_train_processed, y_train, X_val_processed, y_val, epochs, batch_size, validation,
              weight_strategy, upset_weight):
    ...
    if validation == "holdout":
        ...
        for train_index, val_index in tscv.split(X_train_processed):
            X_t_cv = X_train_processed[train_index] if isinstance(X_train_processed, np.ndarray) else \
            X_train_processed.iloc[train_index]
            y_t_cv = y_train.iloc[train_index]
            X_v_cv = X_train_processed[val_index] if isinstance(X_train_processed, np.ndarray) else \
            X_train_processed.iloc[val_index]
...
def run_pytorch_mlp_pipeline(X_train, y_train, X_val, y_val, output_dir, reports_dir,
    ...
    study.optimize(
        lambda trial: objective(trial, X_train_processed, y_train, X_val_processed,
                                y_val, epochs, batch_size,
                                validation, weight_strategy, upset_weight),
        n_trials=n_trials
    )
