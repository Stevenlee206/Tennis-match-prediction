...
from src.models.predictive_coding.pc_layer import PCLayer
...
class PredictiveCodingNetwork:
	...
	def get_state_dict(self) -> dict:
		...
		for i, layer in enumerate(self.layers):
			state[f"W_{i}"] = layer.W.copy()
			state[f"b_{i}"] = layer.b.copy()
		return state

	def load_state_dict(self, state: dict) -> None:
		for i, layer in enumerate(self.layers):
			layer.W = state[f"W_{i}"].copy()
			layer.b = state[f"b_{i}"].copy()

	### UPDATE WEIGHTS ###
	def train_on_batch(self, x0: np.ndarray, y: np.ndarray, sample_weights: np.ndarray = None) -> float:
		...
		# Apply sample weights to output layer error
		if sample_weights is not None:
			w_factor = sample_weights.reshape(-1, 1).astype(np.float32)
			eps[-1] = eps[-1] * w_factor
