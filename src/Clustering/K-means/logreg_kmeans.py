...
def run_logreg_kmeans_search(min_clusters=3, max_clusters=25):
    ...
    # Scale the base features (Mandatory for Clustering and Logistic Regression)
    ...
    plt.plot(k_values, accuracies, marker='o', linewidth=2, markersize=8, color='#d35400', label='LogReg + Clustering Centroid Distances')
    
    # Plot the baseline
    plt.axhline(y=baseline_acc, color='#2c3e50', linestyle='--', linewidth=2, label=f'Baseline LogReg ({baseline_acc:.2f}%)')
    
    # Aesthetics
    plt.title('Logistic Regression Accuracy vs. Clustering Geometric Augmentation', fontsize=16, pad=15)
    ...
    plt.tight_layout()
    plt.savefig('logreg_kmeans_search.png', dpi=300)
