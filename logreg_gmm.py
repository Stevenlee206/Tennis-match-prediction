...
def run_logreg_gmm_search(min_components=3, max_components=25):
    print(f"\n{'='*75}")
    print(f" AUTOMATED GMM LOG-LIKELIHOOD + LOGISTIC REGRESSION SEARCH ({min_components} to {max_components})")
    print(f"{'='*75}")
    ...
    plt.plot(k_values, accuracies, marker='o', linewidth=2, markersize=8, color='#c0392b', label='LogReg + GMM Log-Likelihoods')
    
    # Plot the baseline
    plt.axhline(y=baseline_acc, color='#2c3e50', linestyle='--', linewidth=2, label=f'Baseline LogReg ({baseline_acc:.2f}%)')
    
    # Aesthetics
    plt.title('Logistic Regression Accuracy vs. GMM Probabilistic Augmentation', fontsize=16, pad=15)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'logreg_gmm_search.png', dpi=300)
    print("-> Graph saved successfully as 'logreg_gmm_search.png'!")
    
    plt.show()

if __name__ == "__main__":
    run_logreg_gmm_search(min_components=3, max_components=25)