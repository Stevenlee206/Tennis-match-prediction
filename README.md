...
```powershell
cd "d:\Hoc ML\Tennis-match-prediction"
...
```powershell
python src\execution\main.py --model svm --kernel linear --optimizer optuna --validation holdout --val_size 0.2 --add_pca --add_kmeans --n_clusters 5
```

### Walk-forward mode

```powershell
python src\execution\main.py --model pytorch_svm --optimizer optuna --validation walk_forward --n_splits 5 --torch_opt adam --torch_sched cosine --weight_strategy static --n_trials 30
```
